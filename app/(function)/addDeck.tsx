...
export default function AddDeckScreen() {
  ...
  const handleAddDeck = async () => {
    if (!user)
      return Toast.show({ type: "error", text1: "User not authenticated." });
    if (!deckName)
      return Toast.show({ type: "error", text1: "Deck name is required." });

    try {
      await addDeck(user.uid, deckName, deckDescription, {
        type: visualType,
        key: visualKey,
      });
